...
pub fn process_distribute_rewards(
    ...
    require!(survey.response_count > 0, FormchainError::NoParticipants);

    let participants = ctx.remaining_accounts;
    require!(!participants.is_empty(), FormchainError::NoParticipants);
    require!(
        participants.len() == amounts.len(),
        FormchainError::InvalidRewardType
    );
